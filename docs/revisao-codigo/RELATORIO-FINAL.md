...
| 🔴 C · Idempotência pedido OMIE (timeout pós-commit) | ✅ feito | `1209396`+`2a09ca7` | (1) Escrita no OMIE não retenta (não vira "já cadastrado" por retry). (2) **Heal:** quando o `IncluirPedido` falha, consulta o OMIE por `codigo_pedido_integracao`; se o pedido **realmente existe lá** (resposta perdida), reconcilia (`ENVIADO_OMIE` + número de lá) em vez de falhar; se não existe, propaga o erro real. Nunca reconcilia errado (só confirma). 5 specs. *Shape do `ConsultarPedido` a validar no OMIE real — parse falho → erro original, nunca heal indevido.* |
...
| 🔴 D · Bot 6–8 queries + telefone `contains` sem índice | ✅ feito | `0f8ca68`+`b34d206` | **Cliente** (Inbox→cliente): índice de expressão + `$queryRaw` por igualdade (`0f8ca68`). **Lead** (gate do bot): índice de expressão `Lead_empresaId_telefoneSufixo_idx`; as **duas** buscas de telefone (`fluxoIaConduzindo`+`leadEncerrado`) viraram **UMA** (`buscarLeadDoPeer`, indexada) + avaliação de "encerrado" em memória (`b34d206`). Contato sem lead = 1 lookup indexado que volta vazio na hora. 2 specs novos. |
| 🔴 D · Sync OMIE 2 queries/registro | ✅ feito | `585b63c` | Sync de produtos faz 1 `findMany` em lote por página (estado anterior — preserva contadores + notificação ESTOQUE_ZERADO) + 1 `upsert` por registro, em vez de `findUnique` + `create/update` em série. ~2N → N+1 por página. |
| 🔴 D · Frontend sem cache de dados | ⛔ pendente | — | Não tocado. |
| 🔴 D · Listas grandes sem memo/virtualização | ⛔ pendente | — | Não tocado. |
| 🔴 E · OAuth boilerplate duplicado (6 services) | ✅ feito | `297aab6`+`aa3a08c`+`7bd370f` | **(1) `signState`:** util `@shared/utils/oauth-state.util` (deriveSecret/sign/verify); os 6 services migrados (ML/Shopee/Amazon/TikTok/Meta=eid, Google=uid), salt/claim preservados exatos (`297aab6`). **(2) CredentialStore:** `IntegracoesService.salvarCredenciaisInternas` (par de escrita do `obterCredenciaisInternas`, ponto único D9) — os 5 services de escopo empresa pararam de ter cópia de `new CryptoUtil`+encrypt+upsert+registrarSyncOk; Meta unificou as 2 decifragens na mão no ponto central; `externalAccountId` tipado obrigatório (mata a assimetria `?? undefined`). Review adversária multi-agente do diff sensível. 322 testes verdes (`aa3a08c` piloto ML + `7bd370f` os 4 restantes). |
...
| 🔴 E · `formatMoeda` reimplementado ~30× | ✅ feito | `9085b69`+`0696c2a`+`ef49ca7` | `formatMoeda`/`formatMoedaCompacta` únicos em `lib/masks` (instância `Intl` única). Migradas as 24 cópias: 17× `fmtBRL` + 7× `fmtBRLCompact` (byte-a-byte idênticas), incl. a `PropostaAceitePage`. **Extensão (`ef49ca7`):** + `formatNumero` (substitui ~28× `toLocaleString('pt-BR')` de número) + `formatPercent` (substitui ~13× `${x.toFixed(n)}%` em 16 páginas/2 componentes; troca `.`→`,` = vírgula decimal pt-BR). Datas e 4 valores de export CSV ficam intactos de propósito. tsc + eslint + vite build verdes. |
| 🔴 E · Dois sistemas de estilo + dois diálogos | 🟡 parcial | `63e293f` | Passo seguro feito: `Modal.tsx` marcado `@deprecated`→`ui/Dialog`; convenção no `CLAUDE.md` (ui/ = oficial pra telas novas, styles.ts legado; `@/lib/masks` pra formatação). Diálogos novos já usam `ui/Dialog`. **Icebox:** migração das ~14 páginas que ainda usam o Modal + das 21 que importam `styles.ts`. |
