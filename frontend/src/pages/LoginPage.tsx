...
export default function LoginPage() {
  ...
  // "Esqueceu sua senha?" — abre inline, sem tirar a pessoa da tela de login.
  const [resetAberto, setResetAberto] = useState(false);
  const [resetEnviando, setResetEnviando] = useState(false);
  const [resetAviso, setResetAviso] = useState<string | null>(null);
  ...
  /**
   * Pede o link de redefinição. O backend responde SEMPRE a mesma coisa, exista
   * o e-mail ou não — então a mensagem aqui também não pode diferenciar, senão
   * a tela vira o consultor de "quem tem conta" que o backend se recusa a ser.
   */
  async function pedirReset() {
    const alvo = email.trim();
    if (!alvo) {
      setResetAviso('Digite seu e-mail no campo acima e clique de novo.');
      return;
    }
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* Esqueci a senha — manda o link de verdade (era um mailto) */}
          <div className="flex justify-end mb-2">
            <button
              type="button"
              data-testid="esqueci-senha"
              onClick={() => {
                setResetAberto(true);
                void pedirReset();
              }}
              disabled={resetEnviando}
              className="text-xs hover:underline transition-colors bg-transparent border-0 cursor-pointer p-0"
              style={{ color: COLORS.cyan }}
              onMouseEnter={(e) => (e.currentTarget.style.color = COLORS.magenta)}
              onMouseLeave={(e) => (e.currentTarget.style.color = COLORS.cyan)}
            >
              {resetEnviando ? 'Enviando…' : 'Esqueceu sua senha?'}
            </button>
          </div>
          {resetAberto && resetAviso && (
            <div
              data-testid="esqueci-senha-aviso"
          ...
          {!resetAberto && <div className="mb-3" />}
