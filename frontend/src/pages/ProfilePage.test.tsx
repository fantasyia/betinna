...
vi.mock('@/lib/auth-store', () => ({ getSession: () => ({ user: { id: 'admin-1' } }) }));
...
describe('ProfilePage — diálogos de dinheiro', () => {
  it('salvar comissão manda `comissaoPadrao` (o nome que a API aceita)', () => {
    render(<Pagina />);
    fireEvent.click(screen.getByTestId('user-comissao-btn'));
    fireEvent.change(screen.getByTestId('comissao-input'), { target: { value: '10' } });
    fireEvent.click(screen.getByTestId('comissao-save'));

    expect(put).toHaveBeenCalledWith('/users/rep-1/comissao', { comissaoPadrao: 10 });
  });

  it('salvar teto manda `tetoDesconto`', () => {
    render(<Pagina />);
    fireEvent.click(screen.getByTestId('user-teto-btn'));
    fireEvent.change(screen.getByTestId('teto-input'), { target: { value: '15' } });
    fireEvent.click(screen.getByTestId('teto-save'));

    expect(put).toHaveBeenCalledWith('/users/rep-1/teto-desconto', { tetoDesconto: 15 });
  });
});
