...
- **#17 Float→Decimal no dinheiro — EM ANDAMENTO (Fases 0–4 feitas).** Fase 0: `ResponseInterceptor` converte `Prisma.Decimal → number` em toda resposta (front segue recebendo `number`). **Fase 1** Comissão (totalVendas/totalComissao), **Fase 2** Pedido/PedidoItem (subtotal/total/comissao/precoUnitario), **Fase 3** Proposta/PropostaItem (subtotal/valor/comissaoEstimada/precoUnitario/total), **Fase 4** Preços — Produto (precoTabela/precoFabrica) + ClientePrecoEspecial (precoEspecial), incl. ajuste do `PricingService` (todo cálculo lê com `Number()`) e do catálogo (cast `as` trocado por conversão Decimal→number na fronteira) — todas migradas (`Decimal(14,2)`) + leituras pra `number` envolvidas em `Number()` + teste com `Prisma.Decimal` real. **Faltam:** Lead.valorEstimado, Cliente.limiteCredito, MarketplaceIncident (valor/valorReembolso) — todos de baixo risco (dinheiro mais exibido/comparado que somado). Receita por modelo (provada em 4): schema/migration (`ALTER COLUMN ... TYPE NUMERIC(14,2) USING ...::numeric(14,2)`) + `Number()` só nos sites que o typecheck (`tsc -p tsconfig.build.json`) acusar (`number + Decimal` é erro = rede de segurança) — ⚠️ casts `as` escondem o Decimal do typecheck, revisar à mão + write-paths `Decimal→Decimal` ficam intactos + teste com `Prisma.Decimal`. Manter como Float: % (descontos, comissaoPadrao, markup, percentual), quantidade, posX/posY do FluxoNo. (MovimentoFidelidade.valor = legado morto, pular.)
