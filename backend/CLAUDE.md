...
- **#17 Float→Decimal no dinheiro — EM ANDAMENTO (Fases 0–3 feitas).** Fase 0: `ResponseInterceptor` converte `Prisma.Decimal → number` em toda resposta (front segue recebendo `number`). **Fase 1** Comissão (totalVendas/totalComissao), **Fase 2** Pedido/PedidoItem (subtotal/total/comissao/precoUnitario), **Fase 3** Proposta/PropostaItem (subtotal/valor/comissaoEstimada/precoUnitario/total) — todas migradas (`Decimal(14,2)`) + leituras pra DTO `number` envolvidas em `Number()` + teste com `Prisma.Decimal` real. **Faltam:** preços (Produto precoTabela/precoFabrica + ClientePrecoEspecial precoEspecial — ⚠️ `PricingService` calcula com esses campos, tem ripple), Lead.valorEstimado, Cliente.limiteCredito, MarketplaceIncident (valor/valorReembolso). Receita por modelo (provada em 3): schema/migration (`ALTER COLUMN ... TYPE NUMERIC(14,2) USING ...::numeric(14,2)`) + envolver leitura em `Number()` só nos sites que o typecheck (`tsc -p tsconfig.build.json`) acusar (`number + Decimal` é erro = rede de segurança) + write-paths `Decimal→Decimal` ficam intactos + teste com `Prisma.Decimal`. Manter como Float: % (descontos, comissaoPadrao, markup, percentual), quantidade, posX/posY do FluxoNo. (MovimentoFidelidade.valor = legado morto, pular.)
