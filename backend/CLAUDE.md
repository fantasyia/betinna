...
| D51 | **Shutdown em três camadas, e Redis/Prisma fecham na ÚLTIMA** (`onApplicationShutdown`, não `onModuleDestroy`). Turno de IA que já entregou balões grava `_iaEntregue` no `contexto` da execução (jsonb_set) e a re-execução do mesmo turno NÃO reenvia; chave de idempotência do balão = turno + posição, sem hash do texto. | Medido em 05/09: o deploy mandava SIGTERM, o `RedisService.onModuleDestroy` dava `quit()` na fase 1 enquanto o job do BullMQ ainda rodava (o `@nestjs/bullmq` só drena workers na fase 3) → "Connection is closed." DEPOIS de mandar os balões → passo FALHOU → re-execução no worker novo → resposta re-gerada (nunca sai igual, então hash do texto não dedupava) → cliente lia a despedida duas vezes. Duas defesas independentes: ordem certa de shutdown (causa) e marcador de entrega + chave por posição (efeito, vale pra qualquer re-execução futura). `RAILWAY_DEPLOYMENT_DRAINING_SECONDS=30` nos dois serviços dá o tempo; o `ConversarIaService.onModuleDestroy` drena os turnos do `retomar`. |
...
- [x] **Meta — Facebook Messenger + Instagram Direct** (`@integrations/meta`) — Graph API oficial. `MetaGraphClientService` low-level. `MetaOAuthService` com Facebook Login (state JWT HS256 derivado da `ENCRYPTION_KEY`), troca short-lived → long-lived user token, lista pages do user, vincula IG Business via `instagram_business_account`. Persiste duas `IntegracaoConexao` separadas (servico=`facebook` e `instagram`) com `externalAccountId` = pageId/igUserId (não-cifrado, indexável) pra routing reverso. Webhook único em `/webhooks/meta` (GET handshake com `META_GRAPH_VERIFY_TOKEN` + POST com HMAC SHA-256 do `META_GRAPH_APP_SECRET`). `FacebookService` e `InstagramService` são adapters auto-registrados na `CanalAdapterRegistry`. **Refresh automático do token (auditoria #15):** `MetaTokenRefreshJob` (cron diário 04:30 UTC) renova proativamente o user token long-lived via `MetaOAuthService.renovarTokenSeNecessario` quando faltam <14d pra expirar (Meta estende +~60d e o page token é re-obtido); falha → marca desconectado + alerta o diretor pelo `IntegracaoStatusService`. Endpoints `/integracoes/meta/oauth/{start,callback}` + `/webhooks/meta`. **Lead Ads (formulário nativo, 31/08):** o mesmo webhook recebe `entry.changes[].field='leadgen'`, que NÃO traz dado nenhum do lead — só `leadgen_id`. Por isso o controller apenas ENFILEIRA (`META_LEADGEN_QUEUE`) e o `MetaLeadgenService` busca depois via `GET /{leadgen_id}` com **retry exponencial** (6×30s): token vencido / `leads_retrieval` não aprovada / 5xx do Meta são todos recuperáveis, e desistir na 1ª tentativa perde nome e telefone de um lead PAGO. Idempotência durável = `Lead.variaveis.metaLeadgenId` (o Meta reentrega webhook). Atribuição: `ad_id` → `GET /{ad_id}?fields=campaign{name}` → `utmSource=meta`, `utmMedium=lead_ads`, `utmCampaign=<nome da campanha>`; `ads_read` é permissão SEPARADA de `leads_retrieval`, então a falha aí é best-effort (cai pro `ad_id` cru) — lead sem atribuição vale mais que nenhum lead. **Pré-requisito de operação:** assinar o campo `leadgen` no nível da PÁGINA no app do Meta.
...
- [x] **Fluxos de Automação** (`@modules/fluxos`) — sistema completo com BullMQ (D44):
  ...
  - **CRON_AGENDADO suporta múltiplos horários/regras**: `triggerConfig.expressoes` (array) tem precedência, fallback p/ `expressao` (string legado). `cron.util.ts` valida 5 campos (`validarCronExpr` — cron-parser aceitaria 1 campo como "segundos", daí o bug do "2"); `previewCrons`/`proximaExecucaoCrons` operam no array (funde/ordena preview; cursor = a execução mais cedo entre todas). Front: wizard `montarCrons` (freq por dia c/ N horários, "a cada N min/horas", janela de horário) + construtor visual 5-colunas (`CronVisualBuilder.tsx`) + tradução `cronstrue` pt_BR.
  - **Pular feriados nacionais**: `triggerConfig.pularFeriados` (bool). `feriados.util.ts` calcula feriados federais fixos + móveis (Páscoa via Computus → Sexta-Santa/Carnaval/Corpus Christi) por ano (cache); Consciência Negra (20/11) só ≥2024. Job: no feriado, NÃO dispara mas avança o cursor. `previewCrons(..., pularFeriados)` filtra as datas (cap 2000 iter). NÃO cobre feriados estaduais/municipais.
...
- **✅ #17 Float→Decimal no dinheiro — CONCLUÍDO (Fases 0–5).** Todo dinheiro do app agora é `Decimal(14,2)` (Postgres `NUMERIC`), não mais `Float`. Cobertura: Fase 0 `ResponseInterceptor` (Decimal→number em toda resposta, front intacto); F1 Comissão; F2 Pedido/PedidoItem; F3 Proposta/PropostaItem; F4 Preços (Produto precoTabela/precoFabrica + ClientePrecoEspecial precoEspecial, incl. PricingService e catálogo); F5 Lead.valorEstimado, Cliente.limiteCredito, MarketplaceIncident (valor/valorReembolso). **Seguem como Float de propósito:** % (descontos, comissaoPadrao, markup, percentual, descontoBase), quantidade, posX/posY do FluxoNo. MovimentoFidelidade.valor = legado morto (não usado). **Receita p/ qualquer campo de dinheiro NOVO no futuro:** declarar `Decimal @db.Decimal(14,2)` no schema + migration `ALTER COLUMN ... TYPE NUMERIC(14,2) USING ...::numeric(14,2)` + envolver leitura em `Number()` só onde o typecheck (`tsc -p tsconfig.build.json`) acusar `number + Decimal` (= rede de segurança) — ⚠️ casts `as` escondem o Decimal do typecheck, revisar à mão; write-paths `number→Decimal` ficam intactos (Prisma coage). Padrão de teste: passar `new Prisma.Decimal(...)` no mock e assertar que o retorno é `number`.
- **Resend:** `RESEND_API_KEY` + `RESEND_FROM_EMAIL` **configurados em 29/08** (api + worker; domínio `somatecblocking.com.br` verificado, envio validado). Falta só o **engajamento**: `RESEND_WEBHOOK_SECRET` (signing secret `whsec_` do webhook) + ligar **open/click tracking** no painel do Resend — sem o secret, `/webhooks/resend` recusa TUDO (aceitar sem verificar deixaria qualquer um inflar o engajamento de uma campanha). Backup off-site (R2/S3) opcional via secrets `S3_*` no GitHub (workflow já pula limpo sem eles).
