...
| D45 | **TODAS as integrações de escopo EMPRESA são DIRECTOR-only** (OMIE, WhatsApp empresa, marketplaces ML/Shopee/Amazon/TikTok, social FB/IG). ADMIN NÃO bypassa (diferente do resto do sistema). | Cada integração empresa carrega responsabilidade contratual ou fiscal do decisor da empresa: OMIE (dados fiscais/contábeis), Marketplaces (TOS comerciais, comissões, repasse fiscal), Social (identidade da marca, quem fala em nome da empresa), WhatsApp empresa (risco de ban Meta no número dedicado). Implementado via flag `SERVICO_METADATA.requerDirector` + guard `IntegracoesService.assertDirectorRequerido` (ponto único). ADMIN pode VER status pra debug, mas não conecta/desconecta. **Integrações de escopo USUÁRIO** (google_calendar, sendgrid, openai, anthropic, whatsapp pessoal) NÃO entram aqui — cada user mexe nas suas via `UsuarioIntegracoesService` (que não consulta esta flag). |
